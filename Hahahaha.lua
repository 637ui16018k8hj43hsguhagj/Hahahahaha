-- Luna UI Library v2.0
-- Optimized for Roblox with proper loadstring support
...
function Luna:CreateWindow(config)
    ...
    title.Text = (config.Title or "Luna Hub") .. " | " .. (config.Subtitle or "v2.0")
...
function Tab:CreateToggle(config)
    ...
    local btnCorner = Instance.new("UICorner", switchBtn)
    ...
    function Toggle:Set(value, skipCallback)
        ...
        if self.Window and self.Window.SaveConfig and config.Flag then
            self.Window.ConfigData[config.Flag] = isOn
            self.Window:SaveConfigFile()
        end
    end
    ...
    if config.Flag and self.Window then
        self.Window.Elements[config.Flag] = Toggle
    end
...
function Tab:CreateSlider(config)
    ...
    local function updateSlider(inputPos)
        ...
        if Slider.Window and Slider.Window.SaveConfig and config.Flag then
            Slider.Window.ConfigData[config.Flag] = value
            Slider.Window:SaveConfigFile()
        end
    end
    ...
    if config.Flag and self.Window then
        self.Window.Elements[config.Flag] = Slider
    end
...
function Tab:CreateDropdown(config)
    ...
    function Dropdown:Set(value, skipCallback)
        for i, opt in ipairs(options) do
            if opt == value then
                ...
                if self.Window and self.Window.SaveConfig and config.Flag then
                    self.Window.ConfigData[config.Flag] = value
                    self.Window:SaveConfigFile()
                end
                break
            end
    ...
    button.MouseButton1Click:Connect(function()
        ...
        if self.Window and self.Window.SaveConfig and config.Flag then
            self.Window.ConfigData[config.Flag] = selectedValue
            self.Window:SaveConfigFile()
        end
    end)
    ...
    if config.Flag and self.Window then
        self.Window.Elements[config.Flag] = Dropdown
    end
...
function Tab:CreateMultiDropdown(config)
    ...
    local dropCorner = Instance.new("UICorner", dropdownContainer)
    ...
    local function updateDisplay()
        ...
        if MultiDropdown.Window and MultiDropdown.Window.SaveConfig and config.Flag then
            MultiDropdown.Window.ConfigData[config.Flag] = selected
            MultiDropdown.Window:SaveConfigFile()
        end
    end
    ...
    for _, option in ipairs(config.Options or {}) do
        ...
        local checkCorner = Instance.new("UICorner", checkmark)
    ...
    MultiDropdown.Window = self.Window
    
    if config.Flag and self.Window then
        self.Window.Elements[config.Flag] = MultiDropdown
    end
...
function Tab:CreateTextBox(config)
    ...
    local frameCorner = Instance.new("UICorner", textFrame)
    ...
    textBox.FocusLost:Connect(function(enterPressed)
        ...
        if TextBox.Window and TextBox.Window.SaveConfig and config.Flag then
            TextBox.Window.ConfigData[config.Flag] = textBox.Text
            TextBox.Window:SaveConfigFile()
        end
    ...
    TextBox.Window = self.Window
    
    if config.Flag and self.Window then
        self.Window.Elements[config.Flag] = TextBox
    end
...
function Tab:CreateFolder(name)
    local NewFolder = setmetatable({}, Folder)
    NewFolder.Elements = {}
    NewFolder.Window = self.Window
    ...
    local folderCorner = Instance.new("UICorner", folderContainer)
    ...
    NewFolder.scrollFrame = contentFrame
    
    return NewFolder
end

-- Folder Methods (Same as Tab)
...
-- PART 3/3 COMPLETE - FULL LIBRARY READY!
return Luna
